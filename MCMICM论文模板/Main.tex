...
In order to solve the practical problems related to the propagation of high frequency shortwave signals on the ocean surface, this paper established the attenuation model of ionospheric reflection signals and established the reflection model of electromagnetic waves by using the Fresnel reflection coefficient model and its changing formula. To study the influence of multi-hop HF radio waves on the ionospheric reflections, whether the waves are turbulent on their reflections, the reflection of terrestrial landforms on different terrains, the reception of hull antennas and the characteristics and environment of propagation in the medium.

Under the condition of known signal transmitting angle and frequency, the mathematic model can find out the number of times that the multi-hop HF radio can jump back and forth and the propagation distance through the reflection and propagation losses between the ionosphere and the earth. This paper focuses on the reflection of the ocean surface. When the ocean waves are too large to form a turbulent ocean, the surface reflection problem should be considered as the rough sea surface reflection problem compared with the calm sea surface reflection.

We compare the similarities and differences between the surface reflection contrast of ocean surface and the rugged or flat ground reflection under different environments to analyze the reflection characteristics of different materials under different conditions and also analyze the propagation of HF HFW signals And consumption should also consider the antenna receiving gain, wave shaking and receiving weakness. Other environmental factors also have a role in signal transmission.



\begin{keywords}
multi-hop radio shortwave; sea surface reflection; ionosphere;  \\ \hspace*{1.2cm}ground reflection
\end{keywords}
\end{abstract}
\maketitle
%\pagestyle{empty}
...
Category& Maximum sustained winds & Potential damage \\
 \midrule
  ...
  Category 5 & $ \geq $ 250 km/h &   Most of the buildings and detached houses were completely destroyed, and some houses were blown away completely.Flood have devastated large areas, and all buildings near the coast have been flooded and settlers may need to evacuate. \\ \bottomrule[2pt]
 \end{tabular}
 \end{center} 
 \end{table}
...
\section{Simplifying and Calculating the Model}
...
VRP $^{\cite{Dikas2016Solving,He2015Model}}$ generally defined as: on a range of clients point (location known or can be estimated) in satisfying certain constraints (such as the demand for goods, the delivery time of delivery, the vehicle capacity constraints, etc.), reasonably arrange the vehicle distribution route, making the vehicle through them in an orderly way to achieve a certain goal (such as the shortest mileage and least cost, least time, use as little as possible and so on). The representation of VPR can be seen in Figure \ref{Figure3}.

\begin{figure}[tbp]
  \centering{
  \includegraphics[width=0.8\textwidth]{./picture/bar_demo.png}}
...
Because the evacuation and time of personnel also satisfied the curve of $S$ type curve, it can be used to draw the time-varying personnel evacuation curve of hurricane from category 1 - 5, which can be seen in figure \ref{figure4}.

\begin{figure}[h]
  \centering{
  \includegraphics[width=0.8\textwidth]{./picture/bar3d_demo.png}}
...
\begin{figure}[h]
  \centering{
  \includegraphics[width=0.8\textwidth]{./picture/pie_demo.png}}
...
\lstinputlisting[language=Python]{./code/pca_demo.py}
